...
pub unsafe fn dc_msg_get_summarytext_by_raw(
    ...
    match type_0 {
        20 => {
            prefix = to_cstring(context.stock_str(StockMessage::Image));
        }
        21 => {
            prefix = to_cstring(context.stock_str(StockMessage::Gif));
        }
        50 => {
            prefix = to_cstring(context.stock_str(StockMessage::Video));
        }
        41 => {
            prefix = to_cstring(context.stock_str(StockMessage::VoiceMessage));
        }
        40 | 60 => {
            if dc_param_get_int(param, DC_PARAM_CMD as i32, 0) == 6 {
                ...
                let label = CString::new(
                    context
                        .stock_str(if type_0 == DC_MSG_AUDIO as libc::c_int {
                            StockMessage::Audio
                        } else {
                            StockMessage::File
                        })
